...
public class MemoireDao {
	...
	// méthode de chargement du driver et de connexion à la base
	...
	// méthode d'enregistrement en base du résultat (double)
	public boolean memoriser(Memoire refMemoire) {
		...
		try {
			...
			// si l'enregistrement n'existe pas, on en crée un, sinon, on modifie le premier
	...
	//méthode pour lire la valeur en mémoire
	public Memoire lire() {
		try {
			Connection connexion = connect();
			Statement stmt =connexion.createStatement();
